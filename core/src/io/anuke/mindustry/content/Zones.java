...
public class Zones implements ContentList{
    ...
    public void load(){
        ...
        fieldStrike = new Zone("fieldStrike", new MapGenerator("fieldStrike")){{
            startingItems = list(copper, 1000, silicon, 1000, lead, 1000, scrap, 3000, titanium, 1000, graphite, 1500, metaglass, 1000);
            loadout = Loadouts.basicNucleus;
            conditionWave = 1;
            launchPeriod = 5;
            configureObjective = new Launched(this);
            requirements = with(
                    new ZoneWave(saltFlats, 20),
                    new Unlock(Blocks.daggerFactory)
            );
            resources = with(copper, scrap, lead, coal, sand, titanium, thorium);
        }};
        ...
        Arcticstation = new Zone("ArcticStation", new MapGenerator("ArcticStation", 1)){{
            startingItems = list(scrap, 3000, copper, 1000, lead, 1000, graphite, 300, thorium, 500, silicon, 1000);
            loadout = Loadouts.basicNucleus;
            conditionWave = 15;
            requirements = with(
